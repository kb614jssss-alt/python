product = [
    ...
    {'code':4, 'name':'엘지 세탁기', 'price':2000000},
]
...
while True:
    ...
    if menu =='0':
        ...
        new_name = input('상품이름>')
        new_price = input('상품가격>')
        product.append({'code':new_code,'name':new_name,'plice':new_price})
        print('새로운 상품이 등록되었습니다.')
    elif menu=='3':
        search_name= input('상품이름>')
        for p in product:
            if p['name'].find(search_name) == -1:
                print(p['code'],p['name'],f"{p['prlice']:,}원")
    elif menu== '5':
        ...
        for p in product:
            if int(edit_code)==p['code']:
                ...
                if new_name !='':
                    p['name']=new_name
                #상품가격 수정
                new_price=input(f"가격:{p['prlice']:,}>")
                if new_price != '':
                    p['prlice'] = int(new_price)    